...
En esta práctica se utilizarán los archivos de temeratura mínima y precipitación de Aragón correspondientes a los años 1980 y 2018. Posteriormente se generarán nuevas variables, se extraerán los valores para unas localidades concretas y se visualizarán los reusltados.
...
Para ello, primero extraemos la temperatura y despues añadimos la información existente en la tabla de atributos.  
Temperatura.  
```{r, warning = FALSE, message = FALSE}
Temperatura_minima_anual_PA <- extract(Temperatura_minima_anual, puntos_PA_aragon, fun = mean, df = T)
Temperatura_minima_anual_PA <- cbind(Temperatura_minima_anual_PA, puntos_PA_aragon@data)
```
Precipitación.  
```{r, warning = FALSE, message = FALSE}
Precipitacion_anual_PA <- extract(Precipitacion_anual, puntos_PA_aragon, fun = mean, df = T)
Precipitacion_anual_PA <- cbind(Precipitacion_anual_PA, puntos_PA_aragon@data)
```
...
```
Visualizar
Un ggplot se construye a partir de algunos elementos básicos:  
...
- Cargar datos temperatura y precipitación.   
- Modificar datos para poder realizar un diagrama de cajas.
- Graficar.  
El gráfico final a obtener:

```{r, echo=FALSE}
Grafico_temperatura2 <- ggplot(Temperatura_minima_anual_PA, aes(x = variable, y = value)) + 
...
Grafico_precipitacion2 <- ggplot(Precipitacion_anual_PA, aes(x = variable, y = value)) + 
  geom_boxplot(aes(fill = FIGURA)) +
  scale_x_discrete(labels= c("1980", "2018")) +
  scale_colour_brewer(palette = "Dark2") +
  labs(y = "Precipitación anual (mm)", x = "Año") +
...
ggpubr::ggarrange(Grafico_temperatura2, Grafico_precipitacion2, common.legend = TRUE, legend = "bottom")
```
